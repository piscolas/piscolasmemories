...
public class CrudAutor extends Conexion {
    ...
    public boolean insertar(Autor autor) {
        ...
        String sql = "INSERT INTO autor (nombreAutor,apPaternoAutor,apMaternoAutor,alias,Nacionalidad,Sexo) VALUES(?,?,?,?,?,?)";
        try {
    ...
    public boolean modificar(Autor autor) {
        ...
        Connection con = (Connection) conectar();
        String sql
                = "UPDATE autor SET nombreAutor =  ?,apPaternoAutor =  ?,apMaternoAutor =  ?,alias =  ?, Nacionalidad=?, Sexo=? where  idAutor =  ? ";
        try {
            ps = (PreparedStatement) con.prepareStatement(sql);
            ps.setString(1, autor.getNombre());
            ps.setString(2, autor.getApPaterno());
